...
int lednumpin = 11;
int autopin = 0;
int varNumOfLeds = 40;                                    //variable for reading number of leds
int autox = 0;                                            //auto design cange select, set 1 for auto
int autocount = 0;                                        //auto count variable
int autolimit = 2000;                                      //auto count limit
int changecount = 0;                                      // change the selectpin2 to high when count==10
...
int y1 = EEPROM.read(address);
//int z=y1;
int y2 = EEPROM.read(address + 1);
//sprintf(int(z,y));
//z=z+y2;
int y3 = EEPROM.read(address + 2);
//sprintf(int(z,y));
//z=z+y3;
int y4 = EEPROM.read(address + 3);
//sprintf(int(z,y));
int z = y1 + y2 + y3 + y4;
int stripNumOfLeds = z;                                // the total number of leds

//int h=(stripNumOfLeds/2)+1;
int stripsOn2Pins = false;                                // set to true if the LED strips or rings are connected to 2 input pins
uint32_t stripColor[115];                                  // half of the number of leds + 1
int displayMiddleLed = false;                             // display the middle led (blue). set to true for one strip, false for two strips or rings
int splitStrip = true;                                    // set to true when using 2 strips or rings, false for one strip
int middleOffset = 0;                                     // offset for the middle led when using one strip
int startupAnimationDelay = 6;                            // delay for the startup animation
...
#endif
  delay(200);
  pinMode(lednumpin, INPUT);
  Serial.print("eeprom:");
  Serial.println(stripNumOfLeds, DEC);
  //randomSeed(analogRead(2));
  //Serial.println("exited");


  //read the number of leds
  int lednumselvalue = digitalRead(lednumpin);
  if (lednumselvalue > 0 )
  { Serial.print("read value is:");
    int x = analogRead(brightnessPin);
    x = map(x, 0, 1023, 0, 350);
    //stripNumOfLeds = x;

    Serial.println(x);
    /*   for (int k = 0; k < stripNumOfLeds; k++)
       {
         left_strip.setPixelColor(i, left_strip.Color(0, 150, 0));
       }
       left_strip.show();
       for (int k = 0; k < x; k++)
       {
    ...
    */
    ...
    x = x - 255;
    if (x >= 255)
      EEPROM.update(address + 1, 255);
    else if (x >= 0)
      EEPROM.update(address + 1, x);
    else
      EEPROM.update(address + 1, 0);
    x = x - 255;
    if (x >= 255)
      EEPROM.update(address + 2, 255);
    else if (x >= 0)
      EEPROM.update(address + 2, x);
    else
      EEPROM.update(address + 2, 0);
    x = x - 255;
    if (x >= 255)
      EEPROM.update(address + 3, 255);
    else if (x >= 0)
      EEPROM.update(address + 3, x);
    else
      EEPROM.update(address + 3, 0);
...
void loop() {
  ...
  //automatic design change setup
  autopin = digitalRead(showPeaksPin);
  if (autopin == 1)
  { autocount++;
